...
struct SignInReduce {
    ...
    static var reducer: AnyReducer<States, Action, Environment> {
        return AnyReducer { states, action, environment -> EffectPublisher<SignInReduce.Action, Never> in
            switch action {
            ...
            case .appleSignIn:
                ...
                return environment
                    ...
                    .catchToEffect(Action.handleAppleSingIn)
            ...
            case .fetchKvToken(userFactory: let userFactory, jid: let jid, email: let email, idToken: let idToken):
                return environment.fetchKVToken(userFactory: userFactory
                                                , jid,
                                                email: email,
                                                idToken: idToken).catchToEffect(Action.handleKVTokenFetch)
                
            case .storeAppleUser(let appleUser):
                return .send(.signInProviderID(appleUser: appleUser))
